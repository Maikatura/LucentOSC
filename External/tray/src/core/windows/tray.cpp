...
Tray::Tray::Tray(std::string identifier, Icon icon) : BaseTray(std::move(identifier), icon)
{
    ...
    windowClass.lpszClassName = reinterpret_cast<LPCWSTR>(this->identifier.c_str());
...
void Tray::Tray::exit()
{
    ...
    UnregisterClass(reinterpret_cast<LPCWSTR>(identifier.c_str()), GetModuleHandle(nullptr));
